...
We discovered port 80 http and port 22 ssh. Other than that nothing really interested came up on Zenmap so I decided to check out the website.
Nothing really gave me anything hopefull except the contact page, but after intercepting my "test" request with Burpsuite I realized it went nowhere as well so I decided to start enumerating.

# Enumeration 

I decide to use gobuster for this one and just used the "common.txt" wordlist.
...
I discovered the assets directory which was blank so i decided to see if there was maybe any directory within the assets page so i decied to enumerate it.
...
Found "images" direcorty which I didnt have permissions to view and the "index.php" and PHP means CLI!
...
Which gave me this! I went straigh to Cyberchef and decoded from base64 and go this!
...
Nothing that I can really do with it but that means the my command worked! 
So it's time for PHP shell.
...
Started my netcat listener and ran my shell in the CLI.
...
After that tried to get the user flag but had no permissions so i got stuck for a bit but the remembered the images and thought maybe we got some steganography going on.

Went back to '/var/www' directory and found this Hidden_Content directory in there!
Which had a passphrase.txt with another bas64 encoded password.
...
Got that decoded and went back to get the images back to my machine to use steghide.
...
So we repaired the image and used steghide with our Hidden_Content passphrase and got the password for deku!
...
SSH into the machine with out password and get the user flag!
...
Now it's time for the money shot. We gotta figure out to escalate our privelages.
...
I tried to edit the file but it was unwritable, but the I realised that 'eval' might actually execute my command!
